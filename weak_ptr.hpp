...
namespace softacademy {
    ...
    class WeakPtr {
    public:
        WeakPtr() noexcept : m_ptr(nullptr), m_control(nullptr) {}
        WeakPtr(const SharedPtr<T>& sp) noexcept : m_ptr(sp.m_ptr), m_control(sp.m_control) {
            inc_weak();
        }

        WeakPtr(const WeakPtr& other) noexcept : m_ptr(other.m_ptr), m_control(other.m_control) {
            inc_weak();
        }
        WeakPtr(WeakPtr&& other) noexcept : m_ptr(other.m_ptr), m_control(other.m_control) {
            other.m_ptr = nullptr;
        ...
        void dec_weak() noexcept {
            if (m_control) {
                if (m_control--->weak_count > 0) {
                    m_control--->weak_count;
                    if (m_control->weak_count == 0 && m_control->strong_count == 0) {
                        delete m_control;
                    }
                }
            }
